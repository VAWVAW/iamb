...
fn setup_logging(
    ...
    // `try_init` rather than `set_global_default` because it also installs the
    // `log` compatibility bridge. LiveKit and libwebrtc log through `log`, not
    // `tracing` - including the sink carrying WebRTC's own C++ diagnostics - so
    // without this every word either of them says is discarded, and a call that
    // fails inside the media stack leaves nothing behind to debug with.
    //
    // Their records still go through the filter above, under the targets
    // `livekit`, `livekit::room`, and `libwebrtc`.
    builder.finish().try_init().expect("setting default subscriber failed");
