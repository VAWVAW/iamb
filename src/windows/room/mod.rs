...
impl RoomState {
    ...
    pub fn window_id(&self) -> IambId {
        match self {
            ...
            RoomState::Chat(chat) => {
                IambId::Room(chat.id().to_owned(), chat.thread().cloned().into())
            },
            RoomState::Space(space) => IambId::Room(space.id().to_owned(), RoomView::Main),
            RoomState::Message(msg) => {
                IambId::Room(msg.room_id().to_owned(), RoomView::Message(msg.id().to_owned()))
            },
        }
    }
    ...
    pub fn refresh_room(&mut self, store: &mut ProgramStore) {
        match self {
            ...
            RoomState::Joining(joining) => {
                ...
                *self = RoomState::new(room, RoomView::Main, name, tags, store);
            },
