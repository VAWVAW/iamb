    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        let desc_spans = match (&state.editing, &state.reply_to, state.thread()) {
            (None, None, None) => {
                if state.room.is_tombstoned() {
                    Some(
                        Line::from(vec![
                            Span::styled("This room has been upgraded! ", Style::default().bold()),
                            Span::from("Use "),
                            Span::styled(":follow", Style::default().bold()),
        ...
        let call_line: Option<Line> = {
            ...
            if let Some(incoming) = incoming {
                ...
                let caller = info.map(|info| settings.tunables.get_user_span(&incoming.from, info));
                ...
                for (i, user_id) in participants.iter().take(CALL_BANNER_NAMES).enumerate() {
                    ...
                    let name = info.map(|info| settings.tunables.get_user_span(user_id, info));
                    let name = match name {
                        Some(span) => span.content.into_owned(),
